...
function updateHUD() {
  ...
  document.getElementById('hp-fill').style.background=p.hp<25?'var(--danger)':p.hp<50?'var(--warn)':'';
  document.getElementById('hp-val').textContent=Math.round(Math.max(0,p.hp));
  document.getElementById('en-fill').style.width=`${p.energy}%`;
  document.getElementById('en-val').textContent=Math.round(p.energy);
  document.getElementById('hud-kills').textContent=`${p.kills} kill${p.kills!==1?'s':''}`;
  document.getElementById('hud-score').textContent=`${p.score} pts`;
  // Abilities
  ...
  document.getElementById('zone-phase').textContent=zone.phase===0?'Zone stable':`Zone Phase ${zone.phase}`;
  // Players alive
  const alive=Object.values(remotePlayers).filter(r=>r.alive).length+(p.alive?1:0);
  document.getElementById('players-alive').textContent=`${alive} alive`;
}
...
function addChatMessage(msg) {
  // In-game overlay
  const co=document.getElementById('chat-overlay-msgs');
  if(co) {
  ...
  // Waiting room chat
  const wc=document.getElementById('wait-chat-msgs');
  if(wc) {
    const d=document.createElement('div');
...
class VoiceChat {
  ...
  _updateVCUI() {
    const panel=document.getElementById('vc-players');
    if(!panel) return;
    let html='';
...
function killLocalPlayer() {
  ...
  const timerEl=document.getElementById('resp-timer');
  const timerEl2=document.getElementById('resp-timer2');
  ...
  document.getElementById('resp-ad-btn').onclick=()=>{
    clearInterval(iv);
...
function showRewardedAdDialog(onComplete) {
  ...
  const iv=setInterval(()=>{
    ...
    if(t<=0) {
      ...
      if(btn) { btn.style.display='block'; btn.onclick=()=>{ msg.remove(); onComplete?.(); }; }
    }
  },1000);
}

document.getElementById('rewarded-ad-btn').addEventListener('click',()=>{
...
document.getElementById('ab-pulse-btn').addEventListener('click',activatePulse);
document.getElementById('ab-shield-btn').addEventListener('click',activateShield);
document.getElementById('ab-surge-btn').addEventListener('click',activateSurge);

// Mobile ability buttons
document.getElementById('mb-pulse').addEventListener('touchstart',e=>{e.preventDefault();activatePulse();});
document.getElementById('mb-shield').addEventListener('touchstart',e=>{e.preventDefault();activateShield();});
document.getElementById('mb-surge').addEventListener('touchstart',e=>{e.preventDefault();activateSurge();});
...
function toggleGameChat() {
  ...
  if(!isOpen) { document.getElementById('chat-game-input').focus(); }
}

document.getElementById('chat-game-input').addEventListener('keydown',e=>{
  ...
  if(e.key==='Escape') document.getElementById('chat-input-wrap').classList.remove('open');
  e.stopPropagation();
});
document.getElementById('chat-send-btn').addEventListener('click',()=>{
  const inp=document.getElementById('chat-game-input');
  sendChat(inp.value,'game');
  inp.value='';
  document.getElementById('chat-input-wrap').classList.remove('open');
});
document.getElementById('chat-toggle-btn').addEventListener('click',toggleGameChat);

// Waiting room chat
document.getElementById('wait-chat-send').addEventListener('click',()=>{
  const inp=document.getElementById('wait-chat-input');
  sendChat(inp.value,'wait');
  inp.value='';
});
document.getElementById('wait-chat-input').addEventListener('keydown',e=>{
...
document.getElementById('btn-create').addEventListener('click',async()=>{
  ...
  try{
    ...
    enterWaitingRoom(code);
  }catch(e){showNotif('Error creating room.');}
});
...
document.getElementById('btn-leave2').addEventListener('click',leaveRoom);
document.getElementById('btn-menu-from-results').addEventListener('click',()=>{
...
async function init() {
  ...
  if(!fbOk) {
    setLoadingMsg('⚠️ Configure Firebase in FB_CONFIG (see setup.md)');
    setTimeout(()=>showScreen('s-menu'),2000);
    return;
  }
  try {
    await signIn();
    ...
    setLoadingMsg('Auth failed — check Firebase Anonymous Auth setting');
    setTimeout(()=>showScreen('s-menu'),3000);
    return;
  }
